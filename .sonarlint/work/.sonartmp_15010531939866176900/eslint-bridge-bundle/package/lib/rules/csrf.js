    ...
    create(context) {
        ...
        function isCsurfMiddleware(node) {
            if ((node === null || node === void 0 ? void 0 : node.type) === 'Identifier') {
                node = (0, utils_2.getUniqueWriteUsage)(context, node.name);
            }
            if (node && node.type === 'CallExpression' && node.callee.type === 'Identifier') {
                const module = (0, utils_2.getModuleNameOfIdentifier)(context, node.callee);
        ...
        function checkCallExpression(callExpression) {
            ...
            // require('csurf')
            const requiredModule = (0, utils_2.getModuleNameFromRequire)(callExpression);
            if ((requiredModule === null || requiredModule === void 0 ? void 0 : requiredModule.value) === CSURF_MODULE) {
                importedCsrfMiddleware = true;
            }
            // csurf(...)
            if (callee.type === 'Identifier') {
                const moduleName = (0, utils_2.getModuleNameOfIdentifier)(context, callee);
                if ((moduleName === null || moduleName === void 0 ? void 0 : moduleName.value) === CSURF_MODULE) {
                    const [args] = callExpression.arguments;
                    const ignoredMethods = (0, utils_2.getObjectExpressionProperty)(args, 'ignoreMethods');
                    if (ignoredMethods) {
            ...
            if (callee.type === 'MemberExpression') {
                if ((0, utils_2.isIdentifier)(callee.property, 'use') &&
                    (0, utils_2.flattenArgs)(context, callExpression.arguments).find(isCsurfMiddleware)) {
                    globalCsrfProtection = true;
                }
                if ((0, utils_2.isIdentifier)(callee.property, 'post', 'put', 'delete', 'patch') &&
                    !globalCsrfProtection &&
                    importedCsrfMiddleware &&
                    !callExpression.arguments.some(arg => isCsurfMiddleware(arg))) {
                    context.report({
                        message: (0, utils_1.toEncodedMessage)('Make sure not using CSRF protection is safe here.', []),
                        node: callee,
                    });
                }
            }
