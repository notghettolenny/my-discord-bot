        ...
        return {
            'Program:exit'() {
                ...
                undeclaredIdentifiersByName.forEach((identifiers, name) => {
                    context.report({
                        node: identifiers[0],
                        message: (0, utils_1.toEncodedMessage)(`\"${name}\" does not exist. Change its name or declare it so that its usage doesn't result in a \"ReferenceError\".`, identifiers.slice(1)),
                    });
                });
            },
        };
...
function isWithinWithStatement(node) {
    return !!(0, utils_1.findFirstMatchingAncestor)(node, ancestor => ancestor.type === 'WithStatement');
}
