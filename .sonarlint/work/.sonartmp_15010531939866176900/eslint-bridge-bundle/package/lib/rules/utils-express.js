...
(function (Express) {
    ...
    function attemptFindAppInstantiation(varDecl, context) {
        ...
        if (rhs && rhs.type === 'CallExpression') {
            const { callee } = rhs;
            if (((_a = (0, utils_2.getModuleNameOfNode)(context, callee)) === null || _a === void 0 ? void 0 : _a.value) === EXPRESS) {
    ...
    function attemptFindAppInjection(functionDef, context) {
        ...
        if (app) {
            const parent = (0, utils_2.getParent)(context);
            if ((parent === null || parent === void 0 ? void 0 : parent.type) === 'AssignmentExpression') {
                const { left } = parent;
                if (left.type === 'MemberExpression' &&
                    ((0, utils_2.isModuleExports)(left) || (0, utils_2.isModuleExports)(left.object))) {
                    return app;
                }
            }
        }
    ...
    function isUsingMiddleware(context, callExpression, app, middlewareNodePredicate) {
        if ((0, utils_2.isMethodInvocation)(callExpression, app.name, 'use', 1)) {
            const flattenedArgs = (0, utils_2.flattenArgs)(context, callExpression.arguments);
    ...
    function isMiddlewareInstance(context, middlewares, n) {
        ...
        if (n.type === 'CallExpression') {
            const usedMiddleware = (_a = (0, utils_2.getModuleNameOfNode)(context, n.callee)) === null || _a === void 0 ? void 0 : _a.value;
            if (usedMiddleware) {
                        ...
                        if (app) {
                            ...
                            if (!isSafe) {
                                for (const sensitive of sensitiveProperties) {
                                    context.report({
                                        node: callExpr,
                                        message: (0, utils_1.toEncodedMessage)(message, [sensitive]),
                                    });
                                }
                                sensitiveProperties = [];
                            }
