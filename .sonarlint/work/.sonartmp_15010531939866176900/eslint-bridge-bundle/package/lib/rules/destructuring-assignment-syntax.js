            ...
            declarationsByObject.forEach((declarations, key) => {
                if (declarations.length > 1) {
                    ...
                    if (tail.every(decl => getKind(decl) === firstKind)) {
                        context.report({
                            node: declarations[0],
                            message: (0, utils_1.toEncodedMessage)(`Use destructuring syntax for these assignments from "${key}".`, tail, Array(tail.length).fill('Replace this assignment.')),
                        });
                    }
                }
            });
...
function getKind(declarator) {
    const declaration = (0, utils_1.findFirstMatchingAncestor)(declarator, n => n.type === 'VariableDeclaration');
