...
exports.rule = {
    create(context) {
        ...
        function isInsideTest() {
            return context
                .getAncestors()
                .some(n => n.type === 'CallExpression' && utils_1.Mocha.isTestConstruct(n));
        }
        return {
            ...
            'CatchClause:exit': (node) => {
                ...
                if (param && param.type === 'Identifier') {
                    const exception = (0, reachingDefinitions_1.getVariableFromIdentifier)(param, context.getScope());
            ...
            CallExpression(node) {
                ...
                if (isInsideTest() &&
                    ...
                    (callExpr.arguments.length === 0 ||
                        (callExpr.arguments.length === 1 && (0, utils_1.isIdentifier)(callExpr.arguments[0], 'Error')))) {
                    context.report({
...
function isThrowAssertWithoutNot(node) {
    ...
    if (!(0, utils_1.isIdentifier)(property, 'throw')) {
        return false;
    }
    while (object.type === 'MemberExpression') {
        if ((0, utils_1.isIdentifier)(object.property, 'not')) {
            return false;
        }
        if ((0, utils_1.isIdentifier)(object.property, 'should')) {
            return true;
        }
        object = object.object;
    }
    return object.type === 'CallExpression' && (0, utils_1.isIdentifier)(object.callee, 'expect');
}
