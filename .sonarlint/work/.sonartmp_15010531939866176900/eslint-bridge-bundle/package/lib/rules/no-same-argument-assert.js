...
exports.rule = {
    ...
    create(context) {
        if (!utils_1.Chai.isImported(context)) {
            return {};
        }
        return {
...
function checkAssert(context, expression) {
    if (expression.type === 'CallExpression') {
        ...
        if (callee.type === 'MemberExpression' && (0, utils_1.isIdentifier)(callee.object, 'assert')) {
...
function checkExpect(context, expression) {
    ...
    while (true) {
        ...
        else if ((0, utils_1.isIdentifier)(currentExpression, 'expect')) {
            break;
        }
        else {
            return;
...
function checkShould(context, expression) {
    ...
    while (true) {
        if (currentExpression.type === 'CallExpression') {
            ...
            if ((0, utils_1.isIdentifier)(currentExpression.property, 'should')) {
                hasShould = true;
            }
            currentExpression = currentExpression.object;
        }
        else if ((0, utils_1.isIdentifier)(currentExpression, 'should')) {
            break;
        }
        else if (hasShould) {
...
function findDuplicates(context, args) {
    ...
    for (let i = 0; i < args.length; i++) {
        for (let j = i + 1; j < args.length; j++) {
            ...
            if (duplicates && !(0, utils_1.isLiteral)(args[i])) {
                const message = (0, utils_1.toEncodedMessage)(`Replace this argument or its duplicate.`, [args[j]]);
