...
function checkCallExpression(context, callExpression) {
    ...
    const moduleName = (0, utils_2.getModuleNameOfImportedIdentifier)(context, callee) ||
        (0, utils_2.getModuleNameOfIdentifier)(context, callee);
...
function checkFormidable(context, callExpression) {
    ...
    if (callExpression.arguments.length === 0) {
        const formVariable = (0, utils_2.getLhsVariable)(context);
        if (formVariable) {
    ...
    const options = (0, utils_2.getValueOfExpression)(context, callExpression.arguments[0], 'ObjectExpression');
    if (options) {
        report(context, !!(0, utils_2.getObjectExpressionProperty)(options, UPLOAD_DIR), keepExtensionsValue((_a = (0, utils_2.getObjectExpressionProperty)(options, KEEP_EXTENSIONS)) === null || _a === void 0 ? void 0 : _a.value), callExpression);
    }
}
function checkMulter(context, callExpression) {
    ...
    const multerOptions = (0, utils_2.getValueOfExpression)(context, callExpression.arguments[0], 'ObjectExpression');
    if (!multerOptions) {
        return;
    }
    const storagePropertyValue = (_a = (0, utils_2.getObjectExpressionProperty)(multerOptions, STORAGE_OPTION)) === null || _a === void 0 ? void 0 : _a.value;
    if (storagePropertyValue) {
        const storageValue = (0, utils_2.getValueOfExpression)(context, storagePropertyValue, 'CallExpression');
        if (storageValue) {
...
function getDiskStorageCalleeIfUnsafeStorage(context, storageCreation) {
    ...
    if (args.length > 0 && isMemberWithProperty(callee, 'diskStorage')) {
        const storageOptions = (0, utils_2.getValueOfExpression)(context, args[0], 'ObjectExpression');
        if (storageOptions && !(0, utils_2.getObjectExpressionProperty)(storageOptions, DESTINATION_OPTION)) {
            return callee;
        }
    }
    return false;
...
function getVariablePropertyFromAssignment(context, assignment) {
    ...
    if (memberExpr.object.type === 'Identifier' && memberExpr.property.type === 'Identifier') {
        const objectVariable = (0, utils_2.getVariableFromName)(context, memberExpr.object.name);
        if (objectVariable) {
...
function report(context, uploadDirSet, keepExtensions, callExpression, secondaryLocation) {
    ...
    if (message) {
        if (secondaryLocation) {
            message = (0, utils_1.toEncodedMessage)(message, [secondaryLocation.node], [secondaryLocation.message]);
        }
        else {
