...
exports.rule = {
    create(context) {
        ...
        return {
            CallExpression: (node) => {
                if (utils_1.Mocha.isTestConstruct(node)) {
...
function checkTimeoutDisabling(node, context) {
    if ((0, utils_1.isMethodCall)(node) && node.arguments.length > 0) {
        const { callee: { object, property }, arguments: [value], } = node;
        if ((0, utils_1.isThisExpression)(object) &&
            (0, utils_1.isIdentifier)(property, 'timeout') &&
...
function isDisablingTimeout(timeout, context) {
    const usage = (0, utils_1.getUniqueWriteUsageOrNode)(context, timeout);
    return (0, utils_1.isNumberLiteral)(usage) && usage.value > MAX_DELAY_VALUE;
}
