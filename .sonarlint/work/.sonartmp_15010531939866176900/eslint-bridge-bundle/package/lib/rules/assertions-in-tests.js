    ...
    create(context) {
        ...
        return {
            'CallExpression:exit': (node) => {
                const testCase = utils_1.Mocha.extractTestCase(node);
                if (testCase !== null) {
...
class TestCaseAssertionVisitor {
    ...
    visit(node) {
        ...
        if (utils_1.Chai.isAssertion(node)) {
            this.hasAssertions = true;
            return;
        }
        if ((0, utils_1.isFunctionCall)(node)) {
            const functionDef = (0, utils_1.resolveFunction)(this.context, node.callee);
            if (functionDef) {
                this.visit(functionDef.body);
            }
        }
        for (const child of (0, utils_1.childrenOf)(node, this.visitorKeys)) {
            this.visit(child);
        }
    }
    missingAssertions() {
